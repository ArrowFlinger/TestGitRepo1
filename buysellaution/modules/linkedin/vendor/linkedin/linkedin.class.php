...
class linkedin_core {
	...
	function get_access_token($verifier) {
	        ...
	        $accObj->set_parameter("oauth_verifier", $verifier); # need the verifier too!
		...
		parse_str($output, $oauth);
	        ...
	        $_SESSION['oauth_token'] = $oauth['oauth_token'];
	        $_SESSION['oauth_token_secret'] = $oauth['oauth_token_secret'];
		$_SESSION['laccess']= true;
	}

	function get_profile($requestURL,$config) {
	...
	function get_logged_in_users_profile($config) {
		$this->get_profile(BASE_API_URL . '/v1/people/~',$config);
	}
