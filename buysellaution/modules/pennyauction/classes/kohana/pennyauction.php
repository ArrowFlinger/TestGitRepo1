 ...
 */
class Kohana_Pennyauction extends Controller_Site_Pennyauction {
	
	/**
	* @var array configuration settings
	*/
	protected $_config = array();
	protected $_pennyauction ;
	...
	*/
	...
	public function update_autobid_account()
	{
		...
		$result=$this->pennyauction_model->selectall_autobid_closed();
		...
		foreach($result as $products)
		{
			...
			$this->pennyauction_model->update(USERS,array('user_bid_account'=>$amts),'id',$products['userid']);
			$this->pennyauction_model->delete_autobid($products['userid'],$products['product_id']);
		}
		return;
	}
	...
	{				
		...
		$product_results = $this->pennyauction_model->select_products_detail($pid,$status,$array);			
		$array=array();	
	...
	{
		$pennyauction = Model::factory('pennyauction');		
		$productsresult = $pennyauction->select_products_detail($pids,$status,$arrayset);		
		switch($status)
		{
			case 3:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/future')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 6:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/product_detail')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 2:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/closed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 10:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/winner')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 8:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/closedunclosed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			...
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/closedunclosed')
							...
							break;	
                        case 11:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/buynow')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;		
						
			default:
			$view = View::factory('pennyauction/'.THEME_FOLDER.'auctions/live')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
		}					
