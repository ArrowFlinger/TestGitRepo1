...
class Model_Pennyauction extends Model {
	...
	{
		/**selvam-May 2,2013 for buyerseller**/
		$check_buyerseller=DB::select('typename')
		->from(AUCTIONTYPE)
		...
		$query=DB::select(PRODUCTS.'.product_id',
				...
				USERS.'.user_bid_account',USERS.'.photo')
					...
					//->or_where(PRODUCTS.'.userid','=',0);

					/**selvam-May 2,2013 for buyerseller**/
					/*if(count($check_buyerseller) > 0)
					{
					$query=$query->and_where(PRODUCTS.'.userid','=',0)
								 ->or_where(PRODUCTS.'.userid','>',0);								
					}else{
					$query=$query->and_where(PRODUCTS.'.userid','=',0);									
					}*/
...
// Check Server current time validate 
	...
	public function add_product_form($arr,$settings) 
	{
		...
		->rule ('max_countdown','Model_Adminproduct::maxcountdown',array(":value"))
                ...
                ->rule('bidamount','array_count_equals',array(explode(",",$arr['product_cost']),explode(",",$arr['bidamount']),'current price'))
                ->rule('bidamount','compare_value_greater',array(explode(",",$arr['product_cost']),explode(",",$arr['bidamount']),'current price'))
