 ...
 */
class Controller_Site_Pennyauction extends Controller_Website {	
	...
	{		
		...
		$this->pennyauction_model = Model::factory('pennyauction');	
	...
	{		
		...
		if(is_array($get['pid']))
		{
			...
			*/
			...
			$product_results=$this->pennyauction_model->select_products_user_forbid($id);							
			$used="";
			//Session user id
					...
					{	
						...
						if($bid_count>0)
						{
							if($ses_user!=$product_result['lastbidder_userid'])
							{
								...
								$update=$this->pennyauction_model->update_autobid($ses_user,$id,$bid_count);
								//Update Time stamp for product
								$arr=array('increment_timestamp'=>$time,'current_price'=>$now_price,'lastbidder_userid'=>$ses_user);
								
								$update=$this->auctions->update_product_time(PENNYAUCTION,$arr,$id);
						...
						else
						{
							...
							if($userid!=""){						
								...
								else
								{
									...
									$amt=(Commonfunction::get_user_bonus($userid)+$amt[0]['bid_amount']);									
		...
		else
		{
			...
			*/
			...
			//Select product results with particular id
			$product_results=$this->pennyauction_model->select_products($id);
			...
			$user_results=$this->auctions->select_users($ses_user);
			$auto_bidexists=0;
			//$auto_bidexists=count($this->auctions->select_abid($id));
			//$bid_count=$user_results[0]['user_bid_account'];
				...
				{	
					...
					else
					{
						...
						if($userid=="" || $auto_bidexists==0){		
							...
							{
								$bid_count=$user_results[0]['user_bonus']-($c_price+$product_result['bidamount']);
						                //$bid_count=$this->diff_longfloat($bid_count);
								//$bid_count=$user_results[0]['user_bonus']-$product_result['current_price'];
								//$bid_count=$this->diff_longfloat($bid_count);
					...
					if($bid_count>0)
					{
							...
							if($ses_user!=$product_result['lastbidder_userid'])
							{
								...
								//Update Time stamp for product
								$arr=array('increment_timestamp'=>$time,'current_price'=>$now_price,'lastbidder_userid'=>$ses_user);
								$this->auctions->update_product_time(PENNYAUCTION,$arr,$id);
								...
								else
								{
									$this->commonmodel->insert(BID_HISTORIES,array('user_id' => $ses_user,'product_id'=>$product_result['product_id'],'price'=>$now_price,'bid_type'=>'SB','date'=>$this->getCurrentTimeStamp));
                                                                ...
                                                                * Dec26, 2012
                                                                **/
                                                               $product_settings=$this->pennyauction_model->select_product_settings();
                                                                /*if($product_settings['sms_eachbid']=='Y'){
                                                                        $lastbid_usercontactno=$this->pennyauction_model->select_shipping_phno($product_result['lastbidder_userid']);
	...
	/*public function action_updateproduct()
	{
		...
		$result=$this->pennyauction_model->select_products_to_update($status,$pid,$date);
		...
		if(count($result)>0){
			...
			{	
				...
				if($status!=3)
				{
					...
					$this->pennyauction_model->update(PENNYAUCTION, array('increment_timestamp'=>$timestamp,'product_process'=>LIVE),'product_id',$product_result['product_id']);
					$this->pennyauction_model->update(PRODUCTS, array('in_auction' => 1),'product_id',$product_result['product_id']);
				}
				else
				{
                                        //SMS
                                        if($product_settings['sms_winningbid']=='Y'){
					$lastbid_usercontactno=$this->pennyauction_model->select_shipping_phno($product_result['lastbidder_userid']);
					$mobileno=$lastbid_usercontactno['phoneno'];
					$msg=$product_settings['sms_winningbid_template'];
					$msg=str_replace('##PRODUCT##','Productid :'.strtoupper($product_result['product_id']),$msg);
					$msg=str_replace('##USER##',strtoupper($_SESSION['auction_username']),$msg);
					sms::send_sms($mobileno,$msg);
				}
                                        
					$this->pennyauction_model->update(PENNYAUCTION, array('product_process'=>CLOSED,'enddate' => $date),'product_id',$product_result['product_id']);
					$this->pennyauction_model->update(PRODUCTS, array('enddate' => $date,'in_auction' => 2,'lastbidder_userid' => $product_result['lastbidder_userid'],'current_price' => $product_result['current_price']),'product_id',$product_result['product_id']);
					
					$users=$this->pennyauction_model->getusersab($product_result['product_id']);
