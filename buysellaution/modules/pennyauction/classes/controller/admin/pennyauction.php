 ...
 */
class Controller_Admin_Pennyauction extends Controller_Welcome {	
	...
	{	
		...
		$this->page_title = __('add_penny_auction');                
		$this->selected_page_title = __('add_penny_auction');               
		...
		{		
			...
			$validator =$this->pennyauction_model->add_product_form($post,$this->product_settings);
			   
				  //validation check
				if ($validator->check()) 
				{	
						$status = $this->pennyauction_model->add_product($post,$productid);
						if($status==0)
						{
							//Flash message for sucess product add
							 Message::success(__('penny_auction_added_successfully'));						
		...
		$view = View::factory('admin/penny_auction_add')
				->bind('title',$title)
				->bind('srch',$_POST)
	...
	*/
	public function action_edit()
	{
		...
		$pid = arr::get($_GET,'pid'); 
  		//set page title
		$this->page_title = __('edit_penny_auction');
		$this->selected_page_title = __('edit_penny_auction');		
		...
		$admin_name = $this->session->get("username");				
		//check current action
		$prducts_deteils = $this->pennyauction_model->select_products_foradmin($pid);
		...
		{		            
			...
			{                      
				$post=Arr::extract($_POST, array('startdate','enddate','product_cost','current_price','max_countdown','bidding_countdown','bidamount'));
				$validator = $this->pennyauction_model->edit_product_form($post,$this->product_settings);	 
				
				//validation starts here			 		
				if ($validator->check()) 
				{     
					//product edit process starts here              
					$status = $this->pennyauction_model->edit_product($pid,$post);		
					if($status == SUCESS)
					{
						//Flash message 
