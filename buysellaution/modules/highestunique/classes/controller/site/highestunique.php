 ...
 */
class Controller_Site_Highestunique extends Controller_Website {	
	...
	{		
		...
		if(preg_match('/site\/highestunique\/highest_mybids/i',Request::detect_uri()))
		{
			//Override the template variable decalred in website controller  
                        $this->template=THEME_FOLDER."template_user_sidebar";
                }
       
        $check_highest = $this->highestunique_model->check_highestunique_present();
		if(count($check_highest) <= 0){
	...
	{		
		...
		if(is_array($get['pid']))
		{
			...
			*/
			...
			$product_results=$this->highestunique_model->select_products_user_forbid($id);			
					...
					{	
						...
						if($bid_count>0)
						{
							if($ses_user!=$product_result['lastbidder_userid'])
							{
								...
								$update=$this->highestunique_model->update_autobid($ses_user,$id,$bid_count);
								//Update Time stamp for product
								$arr=array('increment_timestamp'=>$time,'current_price'=>$now_price,'lastbidder_userid'=>$ses_user);
								
								$update=$this->auctions->update_product_time(HIGHESTUNIQUE,$arr,$id);
		...
		else
		{
			...
			*/
			...
			//Select product results with particular id
			$product_results=$this->highestunique_model->select_products($id);
				...
				{	 
					...
					if($bid_count>0)
					{
							...
							{   
								...
								$this->auctions->update_product_time(HIGHESTUNIQUE,$arr,$id);
	...
	public function action_updateproduct()
	{
		...
		$result=$this->highestunique_model->select_products_to_update($status,$pid,$date);
		if(count($result)>0){
			...
			{	
				...
				if($status!=3)
				{
					...
					$this->highestunique_model->update(HIGHESTUNIQUE, array('increment_timestamp'=>$timestamp,'product_process'=>LIVE),'product_id',$product_result['product_id']);
					$this->highestunique_model->update(PRODUCTS, array('in_auction' => 1),'product_id',$product_result['product_id']);
				}
				else
				{
					
					$this->highestunique_model->update(HIGHESTUNIQUE, array('product_process'=>CLOSED,'enddate' => $date),'product_id',$product_result['product_id']);
					$this->highestunique_model->update(PRODUCTS, array('enddate' => $date,'in_auction' => 2,'lastbidder_userid' => $product_result['lastbidder_userid'],'current_price' => $product_result['current_price']),'product_id',$product_result['product_id']);
					
					$users=$this->highestunique_model->getusersab($product_result['product_id']);
	...
	**/
	public function action_bid_history()
	{
		...
		$view = View::factory('highestunique/'.THEME_FOLDER.'auctions/bid_history')
		...
		$bid_unique=$this->highestunique_model->select_bid_unique($id);
		
		$bid_not_unique=$this->highestunique_model->select_bid_not_unique($id);
		//Session uesr
		$bid_history=$this->highestunique_model->select_bid_history($id,$this->auction_userid);
                
		//All usesrs
		$bid_history_all=$this->highestunique_model->select_bid_history_all($id);	
	        //Last insert id
		$last_insertid=$this->highestunique_model->last_insertid($id);	
	...
	**/
	public function action_bid_history_all()
	{
		...
		$view = View::factory('highestunique/'.THEME_FOLDER.'auctions/allusersbidhistory')
					->bind('bid_histories',$bid_history)					
					->bind('user',$this->auction_userid)
					->bind('pagination',$pagination)
					->bind('bid_unique',$bid_unique)
		...
		$bid_not_unique=$this->highestunique_model->select_bid_not_unique($id);
		//Session uesr
		$bid_history=$this->highestunique_model->select_bid_history_all_users($id,$this->auction_userid);
		//All usesrs
		$bid_history_all=$this->highestunique_model->select_bid_history_all($id);	
	        //Last insert id
		$last_insertid=$this->highestunique_model->last_insertid($id);	
	...
	/**
	* Action for users won auctions highest unique bid
	*/
	public function action_unique_auctions()
	{
		...
		$view=View::factory('highestunique/'.THEME_FOLDER.'auctions/unique_won_auctions')
				->bind('users',$users)
		...
		//==========================
			...
			$page_no= isset($page)?$page:0;			
 			$count_user_wonauctions=$this->highestunique_model->count_unique_highest_bidder($this->auction_userid);
		...
		$wonauctions_results=$this->highestunique_model->unique_highest_bidder($offset,REC_PER_PAGE,$this->auction_userid);
	...
	/*My Bids
        *Bid History
         */
	public function action_highest_mybids()
	{
		$this->selected_page_title= __("placed_unique_bid_product");
		$this->is_login();
		$view=View::factory('highestunique/'.THEME_FOLDER.'auctions/highest_mybids')
				->bind('bidhistories',$bidhistory)
				->bind('count_bidhistory',$count_bidhistory)
				->bind('total_count',$total_count)
				->bind('pagination',$pagination);
		
		$count_bidhistory=$this->highestunique_model->select_bids_for_users_highest_unique(NULL,REC_PER_PAGE,$this->auction_userid,TRUE);
		//pagination loads here
			$page=Arr::get($_GET,'page');
			if($page<0)
			{
				$this->url->redirect('users/highest_mybids');
			}
			$page_no= isset($page)?$page:0;
                        if($page_no==0 || $page_no=='index')
                        $page_no = PAGE_NO;
		...
		$bidhistory=$this->highestunique_model->select_bids_for_users_highest_unique($offset,REC_PER_PAGE,$this->auction_userid);	
