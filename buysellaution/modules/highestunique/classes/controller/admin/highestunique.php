 ...
 *
 * @package    highestunique Auction
 * @category   Base
 * @author     Myself Team
 * @copyright  (c) 2012 Myself Team
 * @license    http://kohanaphp.com/license.html
 */
class Controller_Admin_Highestunique extends Controller_Welcome {	
	...
	public function __construct(Request $request, Response $response)
	{
		...
		//$this->_highestunique = new highestunique;
	...
	{	
		...
		$this->page_title = __('add_highest_auction');                
		$this->selected_page_title = __('add_highest_auction');               
		...
		{		
			...
			$validator =$this->highestunique_model->add_product_form($post,$this->product_settings);
			   
				  //validation check
				if ($validator->check()) 
				{	
						$status = $this->highestunique_model->add_product($post,$productid);
						if($status==0)
						{
							//Flash message for sucess product add
							 Message::success(__('highest_auction_added_successfully'));						
		...
		$view = View::factory('admin/highestunique_add')
				->bind('title',$title)
				->bind('srch',$_POST)
	...
	*/
	public function action_edit()
	{
		...
		$pid = arr::get($_GET,'pid'); 
  		//set page title
		$this->page_title = __('edit_highest_auction');
		$this->selected_page_title = __('edit_highest_auction');		
		...
		$admin_name = $this->session->get("username");				
		//check current action
		$prducts_deteils = $this->highestunique_model->select_products_foradmin($pid);
		...
		{		            
			...
			{                      
				$post=Arr::extract($_POST, array('startdate','enddate','product_cost','current_price','max_countdown','bidding_countdown','bidamount'));
				$validator = $this->highestunique_model->edit_product_form($post,$this->product_settings);	 
				
				//validation starts here			 		
				if ($validator->check()) 
				{     
					//product edit process starts here              
					$status = $this->highestunique_model->edit_product($pid,$post);		
					if($status == SUCESS)
					{
						//Flash message 
	  ...
	  /**
        * ****action_Unique highest Bid Price()****
        * Unique highest Bid Price
        */		 
        public function action_unique_highest_bid_price()
	{ 
		        //set page title
			$adminauction=Model::factory('adminauction');
			$this->page_title = __('menu_highest_unique_bid_price');
			$this->selected_page_title = __('menu_highest_unique_bid_price');
			...
			$count_winners_auctions = $this->highestunique_model->count_admin_unique_highest_bidder(); 
		...
		$winner_auction=$this->highestunique_model->unique_admin_highest_bidder($offset,ADM_PER_PAGE);
		
		//pagination ends here
                $view = View::factory('admin/unique_highest_bidder')
                                        ->bind('title',$title)
