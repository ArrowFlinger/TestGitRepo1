...
abstract class Kohana_FB {
			...
			catch(Exception $error) 
			{
				...
				if(strpos($error, 'OAuth') !== FALSE || $this->_facebook->getUser() == 0) 
				{
					if(Kohana::$config->load('fb')->canvas !== FALSE)
					{
						echo "
							<script>
								top.location.href = '{$this->_facebook->getLoginUrl(array(
    'req_perms' => 'email'))}';
							</script>";
					} 
					else 
					{
						header("Location: {$this->_facebook->getLoginUrl(array(
    'req_perms' => 'email'))}");
					}
					exit;
				}
	...
	*/
	...
	{		
		self::instance()->auth();
		$add['access_token']=$this->_facebook->getAccessToken();
		$user_details = $this->_me;
		$add['picture']="https://graph.facebook.com/".$user_details['id']."/picture";
		return Arr::merge($user_details,$add);		
	}

	
	public function publish_fb_share($access_token="",$values=array())
	{
		self::instance()->auth();		
		$access_token=($access_token!="")?$this->_facebook->getAccessToken():$access_token;	
		$attachment=array('access_token' =>$access_token,
				...
				'actions' =>array('name' =>$values['site_name'],'link' =>$values['site_link']));
		try{
			 $this->_facebook->api("/".$this->_facebook->getUser()."/feed",'post',$attachment);
			return TRUE;
		}
		catch (Exception $e){
			return FALSE;
