 ...
 */
class Controller_Admin_Cashback extends Controller_Welcome {	
	...
	{	
		...
		$this->page_title = __('add_cashback_auction');                
		$this->selected_page_title = __('add_cashback_auction');               
		...
		{		
			...
			$validator =$this->cashback_model->add_product_form($post,$this->product_settings);
			   
				  //validation check
				if ($validator->check()) 
				{	
						$status = $this->cashback_model->add_product($post,$productid);
						if($status==0)
						{
							//Flash message for sucess product add
							 Message::success(__('cashback_auction_added_successfully'));						
		...
		$view = View::factory('admin/cashback_auction_add')
				->bind('title',$title)
				->bind('srch',$_POST)
	...
	*/
	public function action_edit()
	{
		...
		$pid = arr::get($_GET,'pid'); 
  		//set page title
		$this->page_title = __('edit_cashback_auction');
		$this->selected_page_title = __('edit_cashback_auction');		
		...
		$admin_name = $this->session->get("username");				
		//check current action
		$prducts_deteils = $this->cashback_model->select_products_foradmin($pid);
		...
		{		            
			...
			{                      
				$post=Arr::extract($_POST, array('startdate','enddate','product_cost','current_price','max_countdown','bidding_countdown','bidamount'));
				$validator = $this->cashback_model->edit_product_form($post,$this->product_settings);	 
				
				//validation starts here			 		
				if ($validator->check()) 
				{     
					//product edit process starts here              
					$status = $this->cashback_model->edit_product($pid,$post);		
					if($status == SUCESS)
					{
						//Flash message 
