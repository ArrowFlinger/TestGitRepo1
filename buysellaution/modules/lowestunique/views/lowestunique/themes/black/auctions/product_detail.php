...
<div class="detail_block">
    ...
    <?php 
    			if(($product_result['product_image'])!="" && file_exists(DOCROOT.PRODUCTS_IMGPATH_THUMB4.$product_result['product_image']))
	       			{ 
...
<div class="product_container clr auction_item" id="auction_<?php echo $id;?>" name="<?php echo URL_BASE;?>auctions/process">
    ...
    <div class="detail-inner clearfix" id="lowest_product_detail_page">
...
<?php if($product_result['product_featured']==HOT){?><span class="hot_icon_product"></span><?php } ?>                    
                    <div class="lowestunique_big"></div>
                    <div id="gallery" class="ad-gallery">
                        <div class="ad-image-wrapper">                        
                         </div>
                         <?php if($product_result['product_process']==CLOSED  && $product_result['lastbidder_userid']!=0){?><img src="<?php echo IMGPATH.SOLD_IMAGE;?>" style="float:left;"class="sold_image"/><?php } ?>                        
					...
					{							 
							foreach($productimage_count as $productallname)
							{
								...
								$product_thumb_size=URL_BASE.PRODUCTS_IMGPATH_THUMB150x150."thumb254x239/".$productallname;	
								$product_small_size_moreimage=URL_BASE.PRODUCTS_IMGPATH_THUMB150x150."thumb81x54/".$productallname;
								?>					
								<li> <a href="<?php echo $product_thumb_size;?>" title="Image"> <img src="<?php echo $product_small_size_moreimage;?>" class="image0"  alt="test" width="81" height="54"  /> </a> </li>					
							<?php 
							}									
					}
					else
        ...
        <div class="history_block clearfix">
            ...
            <div class="timer-block">
                ...
                <div class="timer clearfix">                
                  <strong class="countdown"><?php echo ($product_result['product_process']!=CLOSED)?"<img src=".IMGPATH."ajax-loader2.gif>":__('closed_text');?></strong>
                </div>
                </div>                
            </div>
				...
				<?php if($product_result['buynow_status']==ACTIVE && $product_result['product_process']!='C'):?>                  
				  <div class="buy-left">
                    ...
                    <div class="share clearfix">
                        ...
                        <ul>
                            <li class="twit"><a href="http://twitter.com/share?url=<?php echo $url;?>&amp;text=<?php echo $product_result['product_name'];?>" target="_blank"  title="Twitter" class="fl"></a></li>
                            
                            <li class="face"><a href="https://www.facebook.com/sharer.php?u=<?php echo $url;?>&t=<?php echo $product_result['product_name'];?>" title="Facebook" class="fl" target="_blank"><img src="<?php echo IMGPATH;?>s3.png" alt="Facebook" border="0" /></a></li>
                    ...
                    <div class="clearfix" style="width:309px">                    
                        <div class="selection_input">
                            ...
                            <div class="selection_input_middle">                            
                                <input type="text" name="range1" id="ran" value="" maxlength="6"  title="Enter amount" />
                            </div>
                ...
                <div class="self_single_selection_to">
                    ...
                    <div class="clearfix">
                        <div class="selection_input">
                            <div class="selection_input_left"></div>
                            <div class="selection_input_middle">
                                <input type="text" name="multifrom" maxlength="6"  value="" title="Enter amount" />
                            </div>
                            <div class="selection_input_right"></div>
                        </div>
                        ...
                        <div class="selection_input">
                            <div class="selection_input_left"></div>
                            <div class="selection_input_middle">
                                <input type="text" name="multito" maxlength="6"  value="" title="Enter amount" />
                            </div>
                            <div class="selection_input_right"></div>
                        </div>
	...
	if($first>=0.01 && $first<=200.00){
		...
		if($sec>$first)
		{
			...
			for($i=$first;$i<=$sec;$i+=0.01)
			{
				...
				// add+='<input type="checkbox" class="bid_checkall"><span style="font:bold 12px arial;color:#999;border:1px solid red;"><?php echo __("check_label");?><span>';
				add+='<div class="bidding_options_left">';
				add+='<ul class="clearfix">';
				add+='<li>';
				add+='</li>';
			...
			$('#bidlimits').html("<label style='color:#FFF;'><?php echo __('second_range_greater_than_first_range'); ?></label>");	

			}
	   
			}else{
		
				$('#bidlimits').html("<label style='color:#FFF;'><?php echo __('range_must_be_0.1_to_200'); ?></label>");	
		...
		$('#bidlimits').html("<label style='color:#FFF;'><?php echo __('please_enter_only_numbers'); ?></label>");	
	}
		...
		$('#bidlimits').html("<label style='color:#FFF;'><?php echo __('please_enter_range_values'); ?></label>");	
	}
...
$('html, body').animate({scrollTop:700}, 'slow');
	...
	add+='<form method="post" name="lowestunique" id="bidform" action="">';
		...
		add+='<a href="javascript:;" name="<?php echo URL_BASE; ?>site/lowestunique/bid/<?php echo $product_result["product_id"]; ?>" class="bid" title="<?php echo ($auction_userid == '') ? __("login_label") : __("bid_me_label"); ?>" rel="<?php echo URL_BASE; ?>users/login/" id="<?php echo $product_result["product_id"]; ?>"  data-auctiontype ="<?php echo $product_result["auction_type"]; ?>" data-formid="bidform"> <?php echo __("bid_me_label"); ?></a>';
		add+='</div>';
