 ...
 */
class Kohana_Lowestunique extends Controller_Site_Lowestunique {
	...
	*/
	...
	public function update_autobid_account()
	{
		...
		$result=$this->lowestunique_model->selectall_autobid_closed();
		...
		foreach($result as $products)
		{
			...
			$this->lowestunique_model->update(USERS,array('user_bid_account'=>$amts),'id',$products['userid']);
			$this->lowestunique_model->delete_autobid($products['userid'],$products['product_id']);
		}
		return;
	}
	...
	{				
		...
		$product_results = $this->lowestunique_model->select_products_detail($pid,$status,$array);	
	...
	{
		$lowestunique = Model::factory('lowestunique');		
		$productsresult = $lowestunique->select_products_detail($pids,$status,$arrayset);	
		 
		switch($status)
		{
			case 3:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/future')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 6:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/product_detail')
							->set('productsresult', $productsresult)
							->set('status',$status);							
							break;
			case 2:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/closed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 10:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/winner')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 8:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/closedunclosed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			...
			case 17:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/lowest_unique_bidhistory')
							->set('productsresult', $productsresult)
							->set('status',$status);	
							break;		
                        case 11:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/buynow')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;							
			default:
			$view = View::factory('lowestunique/'.THEME_FOLDER.'auctions/live')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
		}					
