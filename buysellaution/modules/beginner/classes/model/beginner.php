...
class Model_Beginner extends Model {
    ...
    public function select_products_detail($ids,$status=1,$array=array(),$need_count=FALSE)
	{
		...
		{
			$query=$query->and_where(BEGINNER.'.product_process','=',LIVE)->and_where(BEGINNER.".startdate",'<=',$this->getCurrentTimeStamp)->and_where(BEGINNER.".enddate",'>=',$this->getCurrentTimeStamp)->and_where(BEGINNER.'.auction_process','!=','H')->and_where(PRODUCTS.'.buynow_status','=',ACTIVE);
			$result=$query->execute()->as_array();
			return ($need_count)?count($result):$result;
		...
		{
			$query=$query->and_where(BEGINNER.'.product_process','=',LIVE);
		...
		{
			$query=$query->and_where(BEGINNER.'.product_process','=',CLOSED)->where(PRODUCTS.'.in_auction','=',2)->and_where(BEGINNER.'.lastbidder_userid','!=',0);
			$result=$query->execute();
			return ($need_count)?count($result):$result;
		...
		{
			$query=$query->and_where(BEGINNER.'.enddate','<=',$this->getCurrentTimeStamp);
			$result=$query->execute()->as_array();			
			return ($need_count)?count($result):$result;
		...
		{
			$query=$query->and_where(BEGINNER.'.product_process','=',FUTURE);
			$result=$query->execute();
			return ($need_count)?count($result):$result;
		...
		public function select_products($id,$need_count=FALSE)
		{
			...
			$query=DB::select(PRODUCTS.'.product_id',
					...
					USERS.'.user_bid_account',USERS.'.photo')
						...
						->on(PRODUCTS.'.product_id','=',BEGINNER.'.product_id')
						->join(USERS,'left')
						->on(BEGINNER.'.lastbidder_userid','=',USERS.'.id')->where(PRODUCTS.'.product_status','=',ACTIVE);
			if(is_array($id)){
				$query->and_where(BEGINNER.".product_id",'IN',$id);
			}
			else
			{
				$query->and_where(BEGINNER.".product_id",'=',$id);
			}
	...
	*/
	...
	{
		$query=DB::select(BEGINNER.'.product_id',		
				BEGINNER.'.startdate',		
				BEGINNER.'.enddate',
				BEGINNER.'.dedicated_auction',
				BEGINNER.'.lastbidder_userid',
				BEGINNER.'.current_price',
				BEGINNER.'.max_countdown')->from(BEGINNER)	
					->join(PRODUCTS,'left')
					->on(PRODUCTS.'.product_id','=',BEGINNER.'.product_id')
					->where(PRODUCTS.'.product_status','=',ACTIVE);
		...
		{
			case 0://Future
				$query->and_where(BEGINNER.".product_process",'=',FUTURE)
					->and_where(BEGINNER.'.lastbidder_userid','=',0);
				break;			
			case 1://Live
				$query->and_where(BEGINNER.".product_process",'=',LIVE)
						->and_where(BEGINNER.".startdate",'>=',$date)
						->and_where(BEGINNER.".enddate",'<=',$date);
				break;
			case 2://Resume
				$query->and_where(BEGINNER.".startdate",'>=',$date)
					->and_where(BEGINNER.'.lastbidder_userid','!=',0);
				break;
			case 3://Closed
				$query->and_where(BEGINNER.".enddate",'>',$date);
				break;
		}
		$result = $query->and_where(BEGINNER.'.product_id','=',$pid)->limit(1)->execute()->as_array();
		if($need_count)
		{
			return count($result);
		}
	...
	*/  	 
	...
	{	
		...
		$result=$auction->select_with_onecondition(BEGINNER, 'product_id='.$productid);
		...
		//echo $product_process;exit;
		$sql_sel="select count(".PRODUCTS.".product_id) as count FROM ".PRODUCTS.",".BEGINNER." where ".PRODUCTS.".product_id =".BEGINNER.".product_id and ".PRODUCTS.".product_id=$productid";
		$query=DB::query(Database::SELECT,$sql_sel)
				->execute()
				->get('count');
		
		 $sql_query = array('startdate' => $post['startdate'] ,
			   'enddate' =>$post['enddate'],'product_cost' => $post['product_cost'],'current_price' => $post['current_price'],
			   'max_countdown' => $post['max_countdown'] ,'bidding_countdown' => $post['bidding_countdown'], 'bidamount'=> 
				$post['bidamount'],'updated_date' => $mdate,'increment_timestamp' => $increment_time, 'product_process' => $product_process);
		if($query==0)
		{
			$current_date=$this->getCurrentTimeStamp;
			$inauction =1;
		...
		{		
		$update =  DB::update(BEGINNER)->set($sql_query)
				->where('product_id', '=' ,$productid)
				->order_by('updated_date','DESC')
				->execute();
			if($update)
			{
				//echo $inauction."--".$product_process;exit;
   ...
   public function select_products_onclosed($uid,$checkwin=1)
   {
	   $checkwin = $checkwin==1?(1-$checkwin):$checkwin;
	   $query=DB::select(PRODUCTS.'.product_id')
					->from(PRODUCTS)
					->join(BEGINNER,'left')
					->on(PRODUCTS.'.product_id','=',BEGINNER.'.product_id')
					->where(BEGINNER.'.lastbidder_userid','=' ,$uid)
					->and_where(PRODUCTS.'.in_auction','=',2);						
		$result = $query->execute()->as_array();
					
		return count($result)>$checkwin?false:true;
   }
   
   public function select_users_date($uid,$dayscheck=30)
   {
	   $query=DB::select(USERS.'.created_date')
					->from(USERS)
					->where(USERS.'.id','=' ,$uid)->limit(1);						
		$result = $query->execute()->get('created_date');
		return (Commonfunction::getDays($result) > $dayscheck)?false:true;
	...
	*/
	...
	public function update_autobid($uid,$pid,$amt)
	{
		$query=DB::update(AUTOBID)
			->set(array('amount'=>$amt,'time'=>$this->getCurrentTimeStamp))
	...
	public function selectall_autobid_closed()
	{
		$query = DB::select(PRODUCTS.'.dedicated_auction',
							...
							->join(BEGINNER,'left')
							->on(PRODUCTS.'.product_id','=',BEGINNER.'.product_id')
							->where(BEGINNER.'.product_process','=',CLOSED)
							->and_where(PRODUCTS.'.product_status','=',ACTIVE)
							->execute();
						//	print_r($query);
