 ...
 */
class Kohana_Beginner extends Controller_Site_Beginner {
	...
	*/
	...
	public function update_autobid_account()
	{
		...
		$result=$this->beginner_model->selectall_autobid_closed();
		...
		foreach($result as $products)
		{
			...
			$this->beginner_model->update(USERS,array('user_bid_account'=>$amts),'id',$products['userid']);
			$this->beginner_model->delete_autobid($products['userid'],$products['product_id']);
		}
		return;
	}
	...
	{				
		...
		$product_results = $this->beginner_model->select_products_detail($pid,$status,$array);	
		$array=array();	
		foreach($product_results  as $product_result)
		{
			if($this->getCurrentTimeStamp<=$product_result['enddate'])
			{
				...
				$array[]= array("Product"=> array("id"=>$product_result['product_id'],
											...
											"Users"=>array("username"=>ucfirst(Text::limit_chars($product_result['username'],40)),
	...
	public static function product_block($pids,$status="",$arrayset=array())
	{
		...
		$beginner = Model::factory('beginner');		
		$productsresult = $beginner->select_products_detail($pids,$status,$arrayset);
		
		
		switch($status)
		{
			case 3:
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/future')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			case 6:
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/product_detail')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;		
			case 2:
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/closed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;	
			case 10:
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/winner')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;
			//venkatraja added by category products	
			case 7:
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/closedunclosed')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;			
			...
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/closedunclosed')
							...
							break;
                        case 11:
                        $view = View::factory('beginner/'.THEME_FOLDER.'auctions/buynow')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;	
			...
			$view = View::factory('beginner/'.THEME_FOLDER.'auctions/live')
							->set('productsresult', $productsresult)
							->set('status',$status);
							break;			
		}							
