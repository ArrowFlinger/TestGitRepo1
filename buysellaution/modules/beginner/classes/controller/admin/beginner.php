 ...
 * 
 */
class Controller_Admin_Beginner extends Controller_Welcome {	
	...
	{	
		...
		$this->page_title = __('add_beginner_auction');                
		$this->selected_page_title = __('add_beginner_auction');               
		...
		{		
			...
			$validator =$this->beginner_model->add_product_form($post,$this->product_settings);
			   
				  //validation check
				if ($validator->check()) 
				{	
						$status = $this->beginner_model->add_product($post,$productid);
						if($status==0)
						{
							//Flash message for sucess product add
							 Message::success(__('beginnner_added_successfully'));						
		...
		$view = View::factory('admin/beginner_add')
				->bind('title',$title)
				->bind('srch',$_POST)
	...
	*/
	public function action_edit()
	{
		...
		//check current action
		$prducts_deteils = $this->beginner_model->select_products_foradmin($pid);
		...
		{		            
			...
			{                      
				$post=Arr::extract($_POST, array('startdate','enddate','product_cost','current_price','max_countdown','bidding_countdown','bidamount'));
				$validator = $this->beginner_model->edit_product_form($post,$this->product_settings);	 
				
				//validation starts here			 		
				if ($validator->check()) 
				{     
					//product edit process starts here              
					$status = $this->beginner_model->edit_product($pid,$post);		
					if($status == SUCESS)
					{
						//Flash message 
