...
class Controller_Payment extends Controller_Website {
	...
	public function action_process()
	{
		...
		switch($type)
		{
			...
			/**** venkatraja added in 7-March-2013 ***/
			case "product";
				$this->processbuynoworder($finalpost);
				break;
			
			/***venkatraja added end in 7-March-2013**/
	...
	public function processpackageorder($response)
	{
		...
		if(count($response['id'])>0){
			...
			$produc_title_replacevariable = array("##PACKAGE_TITLE##"=>Html::chars($response['name'][0]),"##AMOUNT##"=>$amount);
		...
		$notifyurl =  URL_BASE."paypal/payment/notify";
		
		 $this->payment_success_url = URL_BASE.'paypal/cmspage/page/package-payment-success';
		echo "<br>";
		 $returnurl = URL_BASE.'paypal/payment/checkout';
		echo "<br>";
		 $cancelurl = URL_BASE.'paypal/payment/cancelled';
		echo "<br>";
	...
	*/
		...
		if (Arr::get($customer, 'ACK') === 'Success'){
         ...
         if (Arr::get($paymentresponse, 'ACK') === 'Success') {
            ...
            //redirect to thankyou page   
				...
				$this->request->redirect('paypal/cmspage/page/package-payment-success');
         }else{
	...
	public function action_authorizepayment(){
