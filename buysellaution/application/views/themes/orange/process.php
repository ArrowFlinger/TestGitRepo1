<?php defined("SYSPATH") or die("No direct script access."); ?>
        <div class="wrapper_outer" id="process_page">
			...
			?>
                    ...
                    <div class="container_right">
                        <div class="product_name">
                            ...
                            <p><?php echo (isset($postvalues['name'][$k]))?ucfirst($postvalues['name'][$k]):''; ?></p>
                        </div>
                        <div class="amount">
                            <strong><?php echo __('amount_label');?>:</strong>
                            <p><?php
			    
			    $unitprice=(isset($postvalues['unitprice'][$k]))?$postvalues['unitprice'][$k]:'';
			    echo $site_currency." ".Commonfunction::numberformat($unitprice); ?></p>
                        </div>
		  ...
		  $arr[]=$field;
		     ...
		     if(isset($postvalues['id'][$k]) && isset($postvalues['unitprice'][$k]) && isset($postvalues['quantity'][$k]) && isset($postvalues['name'][$k]) && isset($postvalues['type']) ){
		     
		     if(array_key_exists('shipping_cost',$postvalues)){
			
		$arr[]=array('form[id][]'=>$postvalues['id'][$k],'form[unitprice][]'=>$postvalues['unitprice'][$k],'form[quantity][]' => $postvalues['quantity'][$k],'form[name][]' => $postvalues['name'][$k],'form[shipping_cost]' => $postvalues['shipping_cost'],'form[type]' =>$postvalues['type']);

		     }else{
		$arr[]=array('form[id][]'=>$postvalues['id'][$k],'form[unitprice][]'=>$postvalues['unitprice'][$k],'form[quantity][]' => $postvalues['quantity'][$k],'form[name][]' => $postvalues['name'][$k],'form[type]' =>$postvalues['type']);
		...
		<?php if(count($payment_gateway)>0){?>
                        ...
                        <div class="payment_option">
                            ...
                            <div class="choose_option">
                                ...
                                <div class="input_box_mid">
                                    <div class="choose_option_select">
                                        <select title="<?php echo __('choose_your_option'); ?>" id="gateway_code">
	    ...
	    foreach($payment_gateway as $key){
			...
			if(isset($key['customfunction']) && $key['customfunction']!="")
			{
				    ...
				    $param = array(array($arr),$currency_code,1,$key['paygate_code']);		 
